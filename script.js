    ...
    function obtenerUrlDinamica() {
        return `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?tqx=out:json&gid=0&nocache=${Date.now()}`;
    }
    ...
    async function fetchProductos() {
        try {
            ...
            if (bannerCell && bannerCell.v) {
                // Limpia corchetes o paréntesis por si se pegó en formato Markdown
                const cleanUrl = String(bannerCell.v).replace(/[\[\]\(\)]/g, '').trim();
                document.getElementById('main-banner').style.backgroundImage = `linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url('${cleanUrl}')`;
            }
    ...
    function mostrarProductos(lista) {
        ...
        lista.forEach(p => {
            ...
            div.innerHTML = `
                <div class="product-img-container"><img src="${p.imagen}" class="product-img-tag" onerror="this.style.display='none'"></div>
